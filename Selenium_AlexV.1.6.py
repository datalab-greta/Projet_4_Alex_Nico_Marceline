...
print('Chargé!!!')
time.sleep(3)
##Récupération des codes de fils des discussions
...
for tag in soup.find_all('li', class_='forum-nav-thread'):
    urlsmoocAcces1.append(tag['data-id'])
for tag in soup.find_all('a', class_='topic-title'):
    idcoursaccess.append(tag['data-discussion-id'])
for tag in soup.find_all('li', class_='forum-nav-browse-menu-item'):
    idcoursaccess.append(tag['data-discussion-id'])
print(urlsmoocAcces1)
urlsmoocAcces=pd.DataFrame(urlsmoocAcces1)
urlsmoocAcces.to_csv('Mooc_accessibilité_numérique.csv',index=False, header=False)
time.sleep(1)
